...
*/
class P_stamp_group_controller {
    ...
    function read() {
    	...
    	$sort = getVarClean('sort','str','p_stamp_group_id');
    	$dir  = getVarClean('dir','str','DESC');
    	
        $searchPhrase = getVarClean('searchPhrase', 'str', '');
        $p_stamp_group_id = getVarClean('p_stamp_group_id', 'int', 0);
    	$data = array('items' => array(), 'success' => false, 'message' => '', 'current' => $start, 'rowCount' => $limit, 'total' => 0);

    	try {
            
            $ci = & get_instance();
		    $ci->load->model('pay_param/p_stamp_group');
		    $table = $ci->p_stamp_group;
		    ...
		    if(!empty($searchPhrase)) {
		        $table->setCriteria("( UPPER(stamp_group.code) ".$table->likeOperator." UPPER('%".$searchPhrase."%') )"); 
		    }
...
/* End of file P_stamp_group_controller.php */
/* Location: ./application/libraries/P_stamp_group_controller.php */